...
def main():
    ...
    short = {"t_corrected_ns": "time", "z_axis_intercept_mm": "z-intercept",
             "momentum_gev": "momentum"}
    n1_rows = []
    for label, name in (("z-intercept", "z_axis_intercept_mm"), ("pT", "momentum_gev"),
                        ("time", "t_corrected_ns")):
        others = [short[c] for c in ("t_corrected_ns", "z_axis_intercept_mm", "momentum_gev")
                  if c != name and cuts[c]["enabled"]]
        n1_rows.append([label, " + ".join(others) if others else "none"])
    print_table("N-1 plots - each variable shown with the OTHER cuts applied:",
                ["plots", "cuts applied"], n1_rows, align="ll")
